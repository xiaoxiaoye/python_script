...
split_pattern = re.compile(r"([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{6}) [0-9]+ \[([0-9a-z]+)\] [0-9]+ \[([a-z]+)\] (.*)")
start_pattern = re.compile(r".*>>>>>>>>>>>>>>>>>>dispatch msg start!\[msg_type\:([-0-9]+).*")
end_pattern = re.compile(r".*<<<<<<<<<<<<<<<<<<<dispatch msg end!\[msg_type\:([-0-9]+).*")
...
if __name__ == "__main__":
    ...
    pre_thread_id = None
    thread_lines = {} # {thread_id1: [line1, line2], thread_id2: [line1, line2],...}
    dispatch_messages = {} 
    '''
        msg_type: [
            {
    ...
    with open(file_name, "rb") as fp:
        ...
        while True:
            ...
            try:
                match_flag, log_time, log_thread_id, log_level, log_info = log_split(line)
                if match_flag:
    ...
    if r_search_type == "msg_type":
        if dispatch_messages.get(r_search_value, False):
            messages = sorted(dispatch_messages[r_search_value], key = lambda x : x["msg_time"])
            with open("./log/msg_type_{}.log".format(r_search_value), "wb") as fp:
                for msg in messages:
                    print("app_id: {}, msg_type: {}, receive time: {}, line_no: {}".format(msg.get("app_id", None),msg.get("msg_type", None), msg.get("msg_time", None), msg["log_lines"][0][0]))
                    fp.write("\n=========================================================\n\n")
    ...
    # ! 按app_id筛选日志
        ...
        with open("./log/app_id_{}.log".format(r_search_value), "wb") as fp:
            for app_msg in app_messages:
                print("msg_time:{}, msg_type:{}, line_no:{} in {}".format(app_msg["msg_time"], app_msg["msg_type"], app_msg["log_lines"][0][0], file_name))
                fp.write("\n=========================================================\n\n")
